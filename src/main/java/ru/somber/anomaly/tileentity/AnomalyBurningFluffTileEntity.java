...
public class AnomalyBurningFluffTileEntity extends AbstractAnomalyTileEntity {
    ...
    private BurningFluffEmitter emitter;

    public AnomalyBurningFluffTileEntity(World world, int metadata) {
        super(world, metadata);

        if (world.isRemote) {
            emitter = new BurningFluffEmitter(xCoord + 0.5F, yCoord, zCoord + 0.5F);
